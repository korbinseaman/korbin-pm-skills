...
description: 让合成用户画像在相互隔离的会话中回答定量问卷，支持当前模型或已配置的外部模型，并将原回答汇总到 Excel，同时生成质量报告。本技能不处理访谈。
---
...
- 不读取任务级共享上下文文件；只使用本 Skill 输入契约明确列出的问卷、画像清单和单人画像。
- 每个用户的 Markdown 答卷只作为临时工作文件；最终输出不保留个人答卷原文件。

## 第一步：读取问卷和全量用户清单

主任务只读取并核对：

1. 优先读取 `ur-design-survey` 已确认的 `questionnaire_for_simulator.md`；仅当该文件不存在时回退到 `questionnaire.md` 并记录降噪版缺失。选中问卷必须包含完整题号、题干、选项、量表、必要材料和跳转/结束规则。
2. `<WORK_DIR>/用户调研/personas_data/persons_summary.html`。这是跨课题复用的全局画像清单；读取页面内 `script#persona-summary-data` JSON 的 `total` 和 `persona_ids` 取得人数 `N` 与有序 ID 数组。
...
- 该用户画像绝对路径
- 分配模型与调用方式
- 唯一临时答卷路径：`<TEMP>/ur-user-simulator/<run_id>/answers/<用户ID>.md`
...
python "<SKILL_DIR>/scripts/run_simulation.py" \
  ...
  --persons-summary "<PERSONS_SUMMARY_HTML>" \
  --persons-dir "<PERSONS_DIR>" \
  --config "<LLM_YAML>" \
  --output-dir "<OUTPUT_DIR>"
```

脚本按全量用户 ID 创建独立并行任务。每个工作线程在开始作答时读取任务指定的模拟作答问卷，然后只读取该用户的 TXT 画像；不读取其他画像或其他人的回答。若脚本返回 `fallback_current_model`，改用下面的当前模型流程。
...
全部子任务结束后运行 `native_simulation.py finalize`，传入同一问卷、用户清单、画像目录、临时答卷目录、`run_id` 和质量报告路径。缺失答卷自动记为失败；已存在且用户 ID 正确的临时答卷用于中断恢复，不覆盖重做。

```bash
python "<SKILL_DIR>/scripts/native_simulation.py" finalize \
  --questionnaire "<QUESTIONNAIRE_MD>" \
  --persons-summary "<PERSONS_SUMMARY_HTML>" \
  --persons-dir "<PERSONS_DIR>" \
  --working-answers-dir "<WORKING_ANSWERS_DIR>" \
  --quality-report "<OUTPUT_DIR>/quality_report_<run_id>.md" \
  --run-id "<RUN_ID>"
```
...
主任务同步每个用户的完成状态、实际模型和错误摘要，但不把任务清单、临时答卷或原始模型响应保存为最终文件。成功数加最终失败数必须等于 `N`。
...
- 按 [rules/response_validation_rule.md](rules/response_validation_rule.md) 检查缺失、选项、量表、排他项和问卷中明确写出的显示/跳转规则。
...
按 `persons_summary.html` 的 ID 顺序检查临时答卷。成功用户一人一个 `<用户ID>.md`；失败用户不生成默认答卷，其错误写入质量报告。临时答卷只用于校验和生成 Excel，不属于最终输出。

### 2. 生成 Excel

使用平台电子表格能力读取问卷、全量用户 ID、临时答卷和质量报告，生成 `survey_responses_<run_id>.xlsx`。结构与样式参考 [templates/survey_responses_example.xlsx](templates/survey_responses_example.xlsx)。

- 只创建“问卷回答”工作表，每个用户一行，按全量用户 ID 顺序排列。
- 固定列为“任务ID、用户ID、姓名、实际模型、状态”，之后按问卷顺序设置 `Q1、Q2…`，最后为“错误摘要”。
- 每道题的原回答完整写入对应题号列；某题至少有一人填写回答原因时，在该题后增加 `<题号>_回答原因` 列，全部为空时不创建。
...
```bash
python "<SKILL_DIR>/scripts/native_simulation.py" cleanup \
  --working-answers-dir "<WORKING_ANSWERS_DIR>"
```

只允许清理该次运行位于系统临时根目录下的答卷目录。若 Excel 生成或回读失败，暂不清理，以便重试；此时不能宣告完成。
...
两项必须使用同一个 `run_id`。不得额外输出个人答卷、任务清单、原始 JSON 或结构化 JSON。详细格式见 [references/io-contract.md](references/io-contract.md)。
