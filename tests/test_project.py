...
class WorkflowTests(unittest.TestCase):
    ...
    def test_plan_skill_contains_domain_core_and_substantive_resources(self) -> None:
        ...
        step_1 = skill.index("### 1. 定义调研问题并形成输入摘要")
        first_card = skill.index("#### 用户研究设计确认卡")
        step_2 = skill.index("### 2. 建立目标—证据清单")
        self.assertLess(step_1, first_card)
        self.assertLess(first_card, step_2)
        self.assertLess(first_card, skill.index("## 输出契约"))
        self.assertIn("用户选择“确认”后才进入 Step 2", skill)
        self.assertIn("确认卡是分析结果，不是让用户补写的空表", skill)
        self.assertIn("用户已经明确提供的信息直接写入", skill)
        self.assertIn("2–3 个互斥选项", skill)
        self.assertIn("（推荐）", skill)
        self.assertIn("避免要求用户自由填写", skill)
        ...
        self.assertIn("采用以上推荐方案（推荐）", skill)
        self.assertIn("修改待选择项", skill)
        self.assertIn("重新设计研究思路", skill)
        for example in (
