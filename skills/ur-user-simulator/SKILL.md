...
- 输出目录为 `<WORK_DIR>/output/<YYYYMMDD><课题简称>/survey_response_data/`。
...
2. `ur-generate-personas` 生成的 `persons_summary.txt`。从“画像总数”和“全量用户ID”取得人数 `N` 与有序 ID 数组。
3. 检查 `personas_data/persons/` 中每个 ID 恰好对应一个 `<用户ID>_<姓名>.txt`，且没有额外文件。此时只确定文件路径，不读取所有画像内容；每个子任务执行时再读取自己的画像。
